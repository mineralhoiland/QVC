...
\end{equation}
whenever the physical drive reaches the KT surface before the fold (Section~\ref{sec:vortex_rg}, Theorem~\ref{thm:competing}). In that event the gap would close as a Berezinskii--Kosterlitz--Thouless essential singularity rather than as the square-root fold of the amplitude map. Experimental discrimination between square-root fold softening and essential-singularity collapse is a key test of which infrared surface is realized.

\subsection{Static fold, catalyzon, and Gross--Pitaevskii dynamics}

The static fold, the catalyzon Mexican-hat, and truncated Gross--Pitaevskii evolution probe distinct operators and are not combined as additive percentages of one baseline. They share the same vacuum $E_{\mathrm{vac}}$ and participation $\lambda^\star$. On the amplitude map~\eqref{eq:dimless_gap_sketch}, unit $\lambda$ lies beyond the fold, $\lambda^\star$ sits on the physical branch at $\Delta^\star=\Deltastar$, and the branches merge at $\Delta_c=\Deltac$. The catalyzon channel is the democratic spectrum of $G=g_0(J-I)$ at $g_0^\star=\lambda^\star E_{\mathrm{vac}}$, giving $\Delta_{\mathrm{cat}}=\Delta_0-g_0^\star$ and a well-depth ratio $(\Delta_{\mathrm{cat}}/\Delta_0)^2$ that corresponds to a $\hatbarrier$ barrier reduction at the matched Ginzburg--Landau quartic (Section~\ref{sec:mexican_dict}). The GPE channel uses an attractive $V_{\mathrm{ZPF}}$: over $1$\,ps the live gap \emph{increases} to $\Deltaliveone$ and the corresponding well \emph{deepens}. Increasing $\alpha$ on the static map suppresses $\delta$ because $\ell<0$; that is opposite in sign to BCS pairing, in which an attractive kernel opens a gap. The Landau potential of the catalyzon channel,
\begin{equation}
...
We solve the time-dependent \TQGL{} equation using a split-step Fourier method. The equation of motion actually integrated is the \emph{truncated} Gross--Pitaevskii equation
\begin{equation}
...
\end{align}
The Lorentzian does not vanish at zero frequency in the detuned run ($L(0)=0.20$).
...
\caption{\textbf{DCE amplitude dynamics $A(t)$.} Teal: fold-slaved resonant drive. Coral: detuning-feedback drive (Lorentzian finite at zero frequency). Horizontal markers: $A_c^{\mathrm{fold}}$ (gold dashed) and $A(\lambda^\star)$ (blue dotted). Both drives saturate: $A^*/A_c^{\mathrm{fold}} = \AresAc$ (resonant) and $\AdetAc$ (detuned), confirming the DCE self-limiting theorem. The Lorentzian detuning provides the negative-feedback mechanism that prevents runaway parametric amplification.}
\label{fig:A_vs_time}
\end{figure}
...
A key question for the robustness of the phenomenological catalyzon algebra is how precisely the five model modes must sit at $\omega_0 = 2\Delta/\hbar$. That five-mode count is a resonant-model ansatz, not a Goldstone theorem: truncated \TQGL{} about the hopfion yields a single discrete $U(1)$ mode (Section~\ref{sec:lin_tqgl}). If exact degeneracy of the ansatz were required, the algebra would apply only at fine-tuned parameter values. We show here that the dark eigenvalue of $G=g_0(J-I)$ survives substantial splitting, and propose a self-tuning mechanism by which the condensate can adjust toward near-resonance.
...
The phenomenological coupling matrix $G_{ij}$ (Section~\ref{sec:catalyzon}) has a candidate microscopic origin in Cartan torsion of the hopfion lattice. The $N$-layer TEVC crystal hosts a Weitzenb\"ock connection with torsion tensor~\cite{Nissinen2020,Nissinen2019}:
\begin{equation}
T^{\mu}{}_{\nu\rho} = \frac{\kappa_{\text{tor}}}{4\pi^2}\, \varepsilon^{\mu}{}_{\nu\rho\sigma}\, \partial^{\sigma} q_H,
\label{eq:cartan_torsion}
\end{equation}
where $q_H(\mathbf{r})$ is the local Hopf charge density and $\kappa_{\text{tor}}$ is a dimensionless torsion coupling. A working estimate $\kappa_{\text{tor}}\approx 0.25$ is sometimes quoted as a vertex strength; that number is not used to compute $G_{ij}$ in this paper.
...
\end{equation}
with spectrum $\lambda_{\max}=(N-1)g_0$ (bright; multiplicity~1) and $\lambda_{\min}=-g_0$ (dark/catalyzon; multiplicity~$N-1$). Identifying the prefactor with $\kappa_{\text{tor}} Q_H/(4\pi^2 a_{\mathrm{lat}}^3)$ remains a conjecture and is left for a future three-dimensional Weitzenb\"ock quadrature (Section~\ref{sec:l1_closure}). The Wigner--Seitz $K_0$ kernel of Section~\ref{sec:gij_dict} is a distinct computed overlap.
...
The Topological Dynamic Vacuum Torsion (\TDVT) program shares the hopfion invariant $Q_H=1/N$ with \QVC{} and supplies a candidate microscopic origin for $G_{ij}$. The interface is an action-level coupling together with three analogue platforms. Einstein equations, a cosmological constant, and the identification of spacetime with a hopfion lattice are outlook statements, not inputs of the condensed-matter calculation.

\subsection{How to read the analogue layer}

\begin{itemize}
\item \textbf{Condensed-matter core.} Gap map, frozen-cavity vacuum, catalyzon, truncated \TQGL{}, Hall staircase. This is the content of the present paper.
\item \textbf{Topology and analogue gravity.} Lens-space $Q_H=1/N$; a proposed Cartan-torsion origin of $G_{ij}$ (not evaluated here); the Unruh--Visser acoustic metric and the matching of analogue Hawking temperature to the Schwinger crossover $T_H=T^*$.
\item \textbf{Spacetime ontology (outlook).} A holographic dictionary for \TQGL{} coefficients, and any claim that hopfion phonons derive Einstein dynamics. These identifications are not used as numerical locks.
\end{itemize}
...
The hopfion-spacetime construction treats the \TEVC{} moir\'e superlattice as a conformal boundary of a $(3+1)$-dimensional bulk hopfion crystal. In AdS/CFT-inspired language, each \TQGL{} coefficient would correspond to the boundary value of a bulk field. This section is an outlook matching: it does not derive $\kappa_{\mathrm{GL}}$ or $g_0^\star$ of Section~\ref{sec:l1_dictionary}.

\subsection{Bulk-Boundary Correspondence}

\begin{table}[htbp]
\centering
\caption{Holographic dictionary mapping bulk hopfion fields to boundary \TQGL{} coefficients (outlook matching; not used to lock the condensed-matter coefficients).}
\begin{tabular}{lll}
\toprule
...
\end{equation}
consistent in form with the Peotta--T\"orm\"a formula for flat-band superfluid weight\cite{PeottaTorma2015,Huhtinen2022,Julku2020}. This $\gamma_{\mathrm{QM}}$ is an analogue matching. It is not $\kappa_{\mathrm{GL}}=\kappaGL$ of the matched Ginzburg--Landau set, and it is not imported as a Ginzburg--Landau lock.

%-------------------------------------------------------------------
\section{Acoustic Hawking Temperature and Schwinger Unification}
\label{sec:acoustic_hawking}

The Unruh--Visser acoustic metric for the excitonic superfluid reveals an acoustic surface at the Wigner--Seitz cell boundary. Matching the analogue Hawking temperature to the Schwinger crossover $T^*$ then fixes $R_{\mathrm{WS}}$; the matching is a temperature-scale identification, not a bulk/boundary duality.
...
Setting $T_H = T^*=\Tstar$ (diagnostic with $E_{\mathrm{pair}}=\Epair$) and solving for $R_{\mathrm{WS}}$ is that Hawking--Schwinger matching:
\begin{equation}
...
\subsection{Hawking--Schwinger temperature matching}
...
\end{tikzpicture}
\caption{\textbf{3D Wigner-Seitz cell with acoustic horizon.} The superfluid velocity $v(r) = c_s(r/R_{\text{WS}})^2$ (red arrows, length proportional to speed) reaches the sound velocity $c_s$ at the cell boundary (orange circle, $R_{\text{WS}} =\RwsMatch$), forming an acoustic surface by the matching $T_H=T^*$. The central hopfion core (purple) sits well inside that surface at $r \ll R_{\mathrm{WS}}$.}
...
\begin{enumerate}
    ...
    \item \textbf{\TQGL{} coefficients.} Matched values of $\alpha_{\mathrm{GL}}$, $\beta$, $\kappa_{\mathrm{GL}}$, and $\lambda^\star$ are collected in Section~\ref{sec:l1_dictionary}. A separate healing-scale Hubbard matching (Section~\ref{sec:track_b}) gives $t_{\mathrm{heal}}/U=1$ together with $t_\lambda/U=\tlamoverU$.
    ...
    \item \textbf{Vortex sector.} Periodic THz driving would render the gap map Floquet. Whether a laboratory protocol sits on the fold or the Kosterlitz--Thouless surface remains a joint question of stiffness closure and temperature: if the KT surface is reached first, the gap would close as an essential singularity rather than a square-root fold.
...
The author acknowledges non-dilutive grant support, provided without equity, for engineering, prototyping, and experimental projects. The theoretical analysis reported here received no additional institutional funding.
...
Table~\ref{tab:epistemic} classifies principal results by provenance: geometry inputs; algebraic identities derived in the text; quantities obtained from a stated numerical solver; identifications imposed between sectors; modelling assumptions; and conjectures requiring future derivation or experiment.

\begin{table}[htbp]
\centering
\caption{Status of principal results.}
\small
\begin{tabular}{p{0.46\linewidth}p{0.28\linewidth}l}
\hline
\textbf{Result} & \textbf{Status} & \textbf{Section} \\
\hline
Input parameters ($J$, $\hbar c_s$, $a_H$, $R$, $N$) & Geometry input & \ref{sec:casimir} \\
Bessel mode sum $F(x)=\Flock$ & Numerical & \ref{sec:casimir} \\
Vacuum energy $E_{\mathrm{vac}}=\Evac$ & Numerical on frozen cavity & \ref{sec:casimir} \\
Gap map $\delta=1+\alpha(\ln\delta+\ell)$ & Defining equation & \ref{sec:gap} \\
Phonon-loop IR cutoff vs.\ $\Lambda$ & Numerical (inconsistent as written) & \ref{sec:gap} \\
Fold critical point $\alpha_c=\alphac$ & Algebraic identity & \ref{sec:bridge} \\
Working participation $\lambda^\star=\lstar$ & Working choice & \ref{sec:bridge} \\
Static gap $\Delta^\star=\Deltastar$ & Numerical & \ref{sec:bridge} \\
Cavity $\mathcal{F}_{\text{moir\'{e}}}$ on $A_{\mathrm{geom}}$; $\lambda^\star_F=\lstarF$ & Numerical (cavity channel) & \ref{sec:cavity_qvc} \\
Fractional Hopf charge $Q_H=1/N$ & Algebraic identity & \ref{sec:lens_space_proof} \\
Democratic spectrum $\lambda_{\max}=(N-1)g_0$, $\lambda_{\min}=-g_0$ & Algebraic identity & \ref{sec:catalyzon} \\
Catalyzon gap $\Delta_{\mathrm{cat}}=\Deltacat$; Mexican-hat $\hatbarrier$ & Derived & \ref{sec:l1_dictionary} \\
Live gap $\Delta_{\mathrm{live}}(1\,\mathrm{ps})=\Deltaliveone$ & Numerical & \ref{sec:dynamics} \\
Lindblad $\tau=5$\,ps, frozen $V_{\mathrm{ZPF}}$ ($\Delta<\Delta_c$) & Numerical (unphysical branch) & \ref{sec:lindblad_dict} \\
DCE saturation $A^*/A_c^{\mathrm{fold}}=\AresAc$ & Numerical & \ref{sec:dce_selflimit} \\
Schwinger $\Gamma=\omega_0\alpha^\star e^{-\pi/\alpha^\star}$; $T^*=\Tstar$ & Numerical & \ref{sec:schwinger_twin} \\
Berry ratio $R_{\mathrm{mean}}=\BerryR$ & Numerical & \ref{sec:berry_enhancement} \\
Hall step $\delta\sigma_{xy}=e^2/(2Nh)$ & Derived & \ref{sec:l1_dictionary} \\
Chern--Simons level $k=2N$ & Derived & \ref{sec:l1_dictionary} \\
GL coefficients ($\alpha_{\mathrm{GL}}$, $\beta$, $\kappa_{\mathrm{GL}}$) & Derived / matched & \ref{sec:l1_dictionary} \\
Acoustic matching $T_H=T^*$ fixing $R_{\mathrm{WS}}$ & Temperature-scale matching & \ref{sec:l1_dictionary} \\
Skyrme coefficient $\kappa_S$ & Estimate & \ref{sec:l1_dictionary} \\
Healing-disk $n_0=1/(\pi a_H^2)$ & Modelling assumption & \ref{sec:l1_closure} \\
Five independent hopfion Goldstone modes & Conjecture (not realized on truncation) & \ref{sec:l1_closure} \\
KT-first essential-singularity gap closing & Conjecture & \ref{sec:vortex_rg} \\
Cartan-torsion origin of $G_{ij}$ & Conjecture (not evaluated) & \ref{sec:l1_closure} \\
Devil's staircase fractal structure & Conjecture & \ref{sec:devils_staircase} \\
\hline
\end{tabular}
\label{tab:epistemic}
\end{table}
...
Source code, locked numerical parameters, figure-generation scripts, crystal-structure files, and verification reports accompanying this paper are available at \url{https://github.com/mineralhoiland/QVC}, licensed MIT (software) and CC~BY~4.0 (text and figures). Correspondence: \href{mailto:mineralhoilandphysics@gmail.com}{mineralhoilandphysics@gmail.com}.
