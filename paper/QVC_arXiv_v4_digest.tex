...
\begin{figure}[H]
\centering
\input{tikz/fig_graphical_abstract}
\caption{\textbf{Graphical abstract.} Option~A* freezes the fat-torus geometry and the Tier-C vacuum form, derives the $\alpha$--$E_{\mathrm{vac}}$ bridge, and evaluates the gap on the physical fold branch at working participation $\lambda^\star$. Fold, catalyzon, and truncated Ginzburg--Landau dynamics are parallel channels. Withdrawn $0.244$\,meV and $92\%$ static collapse are not physical outputs of this freeze.}
\label{fig:graphical}
\end{figure}
