...
\begin{tabular}{llll}
\hline
\textbf{Length} & \textbf{Value} & $\eta_{\mathrm{ret}}$ & \textbf{Role} \\
\hline
$a_H=r$ (GL/GPE) & $\aHlock$ & $\etaretaH$ & Input \\
$\xi_{\mathrm{ph}}=\hbar c_s/\Delta_0$ & $\xiph$ & $\etaretph$ & Identity \\
$\xi$ (alternate) & $\xialt$ & $\etaretalt$ & Unused alternate \\
\hline
\end{tabular}
...
\subsection{Ginzburg--Landau coefficients}
\label{sec:gl_dict}
...
\end{equation}
Hence $m^*=\hbar^2/(2\kappa_{\mathrm{GL}})=\mstaroverme\,m_e$. This $\kappa_{\mathrm{GL}}$ is not the holographic $\gamma_{\mathrm{QM}}$ of Section~\ref{sec:holographic_dict}; that coefficient is an analogue matching and is not imported into the Ginzburg--Landau set.
...
The Skyrme coefficient that stabilizes finite-size textures against Derrick collapse is not derived from a microscopic Hubbard model. A Faddeev--Skyrme balance at the locked tube scale $a_H$ gives the estimate $\kappa_S\sim\kappa_{\mathrm{GL}}a_H^2=\Delta_0 a_H^4=\kappaSansatz$. Table~\ref{tab:gl_dict} collects the matched Ginzburg--Landau coefficients.

\begin{table}[htbp]
\centering
\caption{Matched Ginzburg--Landau coefficients. The gap-map $\alpha$ is dimensionless; $\alpha_{\mathrm{GL}}$ and $\beta$ are energies. The holographic $\gamma_{\mathrm{QM}}$ is not $\kappa_{\mathrm{GL}}$.}
\begin{tabular}{llll}
\hline
\textbf{Symbol} & \textbf{Value} & \textbf{Meaning} & \textbf{Role} \\
\hline
$\alpha^\star$ & $\alphastar$ & Gap-map $\lambda^\star E_{\mathrm{vac}}/\Delta_0$ & Derived \\
$\alpha_{\mathrm{GL}}$ & $\alphaGL$ & Landau mass, $|\alpha_{\mathrm{GL}}|=\Delta_0$ & Matched \\
$\kappa_{\mathrm{GL}}$ & $\kappaGL$ & $\Delta_0 a_H^2$ with $a_H=r$ & Derived \\
$m^*/m_e$ & $\mstaroverme$ & $\hbar^2/(2\kappa_{\mathrm{GL}})$ & Derived \\
$\lambda^\star$ & $\lstar$ & Working participation & Working choice \\
$\beta$ & $\betaGL$ & $\Delta_0/n_{\mathrm{ref}}$ on hopfion initial data & Matched \\
$n_{\mathrm{ref}}$ & $\nrefhopf$ & Ring-weighted hopfion density & Numerical \\
$\kappa_S$ & $\kappaSansatz$ & Derrick estimate at tube scale & Estimate \\
\hline
\end{tabular}
\label{tab:gl_dict}
\end{table}
...
The bulk potential of the matched Ginzburg--Landau set is the quadratic-quartic family
\begin{equation}
...
The Unruh--Visser acoustic metric of Section~\ref{sec:acoustic_hawking} has surface gravity $\kappa_{\mathrm{surface}}=2c_s/R_{\mathrm{WS}}$ and Hawking temperature $T_H=\hbar c_s/(\pi k_B R_{\mathrm{WS}})$. The analogue Hawking--Schwinger matching $T_H=T^*$ then inverts for the Wigner--Seitz radius
\begin{equation}
...
\specGring.
\end{equation}
Converting either $g_{0,\mathrm{eff}}$ to meV requires a prefactor that is not supplied by the overlap; the meV scale of the catalyzon remains the bridge value $g_0^\star$. Neither kernel is a torsion vertex (a three-dimensional Weitzenb\"ock quadrature is left for future work; Section~\ref{sec:l1_closure}) nor a hand-inserted $J-I$.
...
\begin{tabular}{lll}
\hline
$\tau_{\mathrm{dec}}$ & $\Delta_{\mathrm{live}}$ & $g^{(1)}$ \\
\hline
coherent & $\Deltaliveone$ & $\goneps$ \\
$5$\,ps & $\Deltataufive$ & $\gonetaufive$ \\
$20$\,ps & $\Deltatautwenty$ & $\gonetautwenty$ \\
$50$\,ps & $\Deltataufifty$ & $\gonetaufifty$ \\
\hline
\end{tabular}
