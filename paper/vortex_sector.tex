...
\label{eq:polar}
\end{equation}
The locked gap map of Section~\ref{sec:gap} lives in the amplitude $\rho$. The vortex sector of the theory lives in the phase $\theta$. Together the two sectors furnish a competing-scale structure: a saddle-node fold in $\rho$, and a Kosterlitz--Thouless unbinding surface in the stiffness of $\theta$. When a physical drive reaches the KT surface first, the implied gap closing is a Berezinskii--Kosterlitz--Thouless essential singularity. That implication is part of the theory; it is evaluated below by the stiffness, vortex-core, and correlator calculations of this section.

The working fat-torus geometry freezes the toroidal cavity, so the vacuum amplitude---and therefore $\alpha$---does not run with the KT scale $\ell=\ln(L/a_c)$. The suite therefore scans frozen $\alpha$ along the locked physical branch $\delta_+(\alpha)$. Kosterlitz's coupling is $K=\kappa/T_{\mathrm{eff}}$, with critical value $K_c=2/\pi$. No bare transition temperature is inserted by hand, and no chain-rule $\beta_\alpha$ is integrated.
...
\subsection{Isolated winding-one vortex core}
\label{sec:c2_core}
...
\label{eq:kt_surface}
\end{equation}
with $\Delta\to 0$ only if $\Delta$ tracks a positive power of $\kappa$ after unbinding. The vortex-first essential-singularity implication holds if and only if the physical drive path hits the KT surface \emph{before} the fold. If $\kappa(A)$ remains larger than $\kappa_{\mathrm{BKT}}$ at $\alpha=\alpha_c$ and finite-$y$ flow stays bound, the fold wins and the gap map retains a finite $\Delta_c$.
\end{theorem}

Whether a given protocol satisfies it is a property of $\kappa(\alpha)$, $E_{\mathrm{core}}/\kappa$, and $T_{\mathrm{eff}}$.

\begin{hypothesis}[Vortex-first essential singularity]
...
This is a conjecture. The phase-stiffness estimator is a computed closure with modelling assumptions, not a Brillouin-zone theorem, so the competing-scale diagram below is the implication of that closure.
...
\subsection{Competing-scale scan}
...
\begin{figure}[htbp]
\centering
\input{tikz/fig_vortex_rg_flow}
\caption{\textbf{Kosterlitz flow at frozen $\alpha$ and $T^*$.} Primary-closure trajectories at $\alpha/\alpha_c=0.30$, at $\lambda^\star$, and near the fold all run to $y\to 0$ with $K\gg 2/\pi$ (bound). A conventional-only station at the finite-$y$ onset $\alpha/\alpha_c\approx 0.945$ runs to a plasma. The working cavity holds $\alpha$ fixed; there is no $\beta_\alpha$.}
\label{fig:vrg_flow}
\end{figure}

\begin{figure}[htbp]
\centering
\input{tikz/fig_competing_scales}
\caption{\textbf{Competing-scale diagram at $T^*$.} Kosterlitz $K=\kappa/T^*$ along the locked physical branch for the primary stiffness closure and the conventional-only variant. The bosonic line $K=2/\pi$ and the labeled anyon line lie far below the primary curve; the fold is reached first. Conventional-only unbinding is a finite-$y$ infrared plasma at $\alpha/\alpha_c\approx 0.945$, marked, not a bare $K<2/\pi$ crossing.}
\label{fig:competing_scales}
\end{figure}

\begin{figure}[htbp]
\centering
\input{tikz/fig_teff_window}
...
\subsection{Vacuum driving and the essential-singularity implication}
...
\begin{hypothesis}[Overdamped vortex mobility]
...
Two labeled mobility estimates are compared with the primary $\kappa(\alpha)$ at $T^*$. The phonon amplitude already present in the truncated GPE is $\hbar/\tau_v\sim g_{\mathrm{ph}}=0.1\Delta_0$. An Eliashberg-type order-of-magnitude estimate with $\lambda_{\mathrm{ep}}=1$ is $\hbar/\tau_v\sim 2\pi\lambda_{\mathrm{ep}}k_BT$. Along the primary branch both give $(\hbar/\tau_v)/(\pi\kappa)\sim 0.04$--$0.19<1$: the vortices are underdamped. A sharp stiffness jump could therefore survive if a KT surface were hit. Overdamped mobility is not forced by these estimates; it remains a conjecture pending a microscopically computed vortex mobility.
...
\begin{table}[htbp]
\centering
\caption{Fold versus vortex-RG observables. Confirming the vortex-first essential singularity on a given protocol requires the essential-singularity comparison on vortex-inclusive data over $\gtrsim 1.5$ decades in $t_A=(A_c-A)/A_c$, together with $\eta\to 1/4$ in $g(r)$.}
\label{tab:vrg_obs}
\small
\begin{tabular}{@{}llll@{}}
\toprule
Observable & Fold (established) & Vortex RG (conjectured) & Present calculation \\
\midrule
$\Delta(A)$ & ends at $\Delta_c$; $\sqrt{t_A}$ & $\Delta\to 0$; $e^{-C/\sqrt{t_A}}$ & Bound at $T^*$; fold data prefer $\sqrt{t_A}$ \\
$\kappa$ & need not jump & universal jump~\eqref{eq:universal_jump} & Two-sector $\kappa(\alpha)$ \\
$g(r)$ & amplitude death first & algebraic $\to$ exponential & spin-wave at primary $K$ \\
vortex density & not required & proliferates above $A_c$ & Isolated core; plasma flag \\
mobility & --- & jump vs AHNS crossover & Underdamped at $T^*$ \\
specific heat & mean-field anomaly possible & no latent heat & Not computed \\
\bottomrule
\end{tabular}
\end{table}

The vortex-RG sector is closed by the following statements. The Villain reduction from the \TQGL{} functional is a modelling assumption. The stiffness identity~\eqref{eq:kappa} is derived given the healing-disk $n_0$ and the frozen geometric floor, both of which are modelling assumptions. The vortex-first essential singularity is a conjecture: if the drive path satisfies Theorem~\ref{thm:competing}, the gap closing takes the essential-singularity form~\eqref{eq:v9}. On the primary closure at $T^*$ the fold is reached first; conventional-only stiffness, and the primary closure at $T_{\mathrm{eff}}\sim 0.5$--$1$\,K, populate the KT side of the diagram as finite-$y$ plasmas. Chern--Simons remains truncated from the default scan unless Eq.~\eqref{eq:Kc_anyon} is restored as a labeled option. The locked amplitude fold is the mean-field backbone of the static theory and is independent of whether a given protocol realizes the vortex-first scenario.
